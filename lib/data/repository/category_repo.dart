...
class CategoryRepo{
  ...
  Future<http.StreamedResponse> addCategory(String categoryName,int categoryId,  XFile file, String token, {bool isUpdate = false}) async {
    http.MultipartRequest request = isUpdate ? http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.UPDATE_CATEGORY_URI}')):
    http.MultipartRequest('POST', Uri.parse('${AppConstants.BASE_URL}${AppConstants.ADD_CATEGORY}'));
